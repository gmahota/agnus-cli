...
function renderTasks(tasks, title, showDate) {
  ...
  if (showDate) {
    ...
    const tasksAtDates = Array.from(tasksAtDatesMap, (taskAtDate) => ({
      [taskAtDate[0]]: taskAtDate[1],
    }));

    const tasksSortedByDate = tasksAtDates.sort((a, b) => {
      const dateA = new Date(Object.keys(a)[0]).getTime();
      const dateB = new Date(Object.keys(b)[0]).getTime();
      return dateA - dateB;
    });
  ...
  if (tasks.length !== 0) {
    const mappedTasks = tasks.map(
      (task) =>
        `  ${STATISTICS(`${task.id}.`)} ${
          itemSymbol[task.status]
        } ${descriptionColor[task.status](task.description)} ${TIME_STAMP(
          task.age
        )}${task.status === "pending" ? priorityColor[task.priority] : ""}`
    );
  ...
  console.log(
    `${TICK(completedTasks)} ${STATISTICS(`done -`)} ${UNTICKED(
      pendingTasks
    )} ${STATISTICS(`pending`)}`
  );
}
